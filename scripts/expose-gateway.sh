...
for LBL in gateway.envoyproxy.io/owning-gateway-name gateway.networking.k8s.io/gateway-name gateway.networking.k8s.io/owning-gateway-name; do
  SEL="$LBL"
  [[ -n "$GW" ]] && SEL="$LBL=$GW"
  SVC=$(kubectl --context "$CONTEXT" -n "$NS" get svc -l "$SEL" -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)
  [[ -n "$SVC" ]] && break
done

if [[ -z "$SVC" ]]; then
  SVC=$(kubectl --context "$CONTEXT" -n "$NS" get svc -o json 2>/dev/null | python3 -c 'import json,sys; items=[s["metadata"]["name"] for s in json.load(sys.stdin)["items"] if s["spec"]["type"] in ("LoadBalancer","NodePort")]; print(items[0] if items else "")')
fi
[[ -n "$SVC" ]] || { echo "no gateway Service found in $CONTEXT/$NS" >&2; exit 1; }

kubectl --context "$CONTEXT" -n "$NS" patch svc "$SVC" --type=merge -p '{"spec":{"type":"NodePort"}}' >/dev/null
PATCH=$(kubectl --context "$CONTEXT" -n "$NS" get svc "$SVC" -o json | python3 -c '
import json, sys
...
[[ "$PATCH" == "[]" ]] || kubectl --context "$CONTEXT" -n "$NS" patch svc "$SVC" --type=json -p "$PATCH" >/dev/null

kubectl --context "$CONTEXT" -n "$NS" get svc "$SVC" -o jsonpath='{range .spec.ports[*]}{.name}{" port="}{.port}{" nodePort="}{.nodePort}{"\n"}{end}'
